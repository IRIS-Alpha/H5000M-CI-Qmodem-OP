...
> 
> 感谢作者开发的 QModem 蜂窝模组综合管理系统。本固件集成 **luci-app-qmodem** 核心界面，提供模组监控、拨号管理与 AT 调试能力。
...
FUjr [QModem](https://github.com/FUjr/QModem) 的 LuCI 管理前端，与 QModem 核心脚本配套使用，可在 LuCI 的“网络”菜单中显示。
...
* **⚠️ 使用提示**：本固件同时提供 `luci-app-qmodem-generic` 与 `luci-app-qmodem` 两个界面，二者均通过 QModem 管理模组，建议避免同时执行拨号或 AT 操作。未启用 QModem 的 MWAN、TTL 等可选扩展。
